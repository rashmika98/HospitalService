...
public class HospitalsAPI extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String output = HospitalObj.insertHospital(request.getParameter("HRegID"), request.getParameter("HName"),
				request.getParameter("HAddress"), request.getParameter("HCity"), request.getParameter("HDestrict"),
				request.getParameter("HProvince"), request.getParameter("HEmail"), request.getParameter("HContactNum"),
				request.getParameter("HUsername"), request.getParameter("HPassword"));
	...
	protected void doPut(HttpServletRequest request, HttpServletResponse response)
		...
		String output = HospitalObj.updateHospital(paras.get("hidHospitalIDSave").toString(),
				...
				paras.get("HAddress").toString().replace("%2C", ",").replace("%2F", "/").replace("%3A", ":")
						.replace("+", " "),
				paras.get("HCity").toString().replace("+", " "), paras.get("HDestrict").toString().replace("+", " "),
				paras.get("HProvince").toString().replace("+", " "), paras.get("HEmail").toString().replace("%40", "@"),
				paras.get("HContactNum").toString(), paras.get("HUsername").toString(),
				paras.get("HPassword").toString().replace("%40", "@").replace("%23", "#").replace("%24", "$"));
		response.getWriter().write(output);
	}
